...
let currentPage = 1;
let itemsPerPage = 5;
let totalCount = 0; // 전체 URL/메모 개수
let searchQuery = ""; // 현재 검색어
let currentMode = "url"; // 현재 모드: 'url' 또는 'memo'
...
function setupEventListeners() {
  ...
  document.addEventListener("click", (e) => {
    ...
    if (
      ...
      !elements.toolsDropdown.contains(e.target)
    ) {
      closeToolsMenu();
    }
  });
  ...
  document
    .querySelectorAll('[data-dismiss="duplicate-modal"]')
    .forEach((btn) => {
  ...
  document
    .querySelectorAll('[data-dismiss="bulk-delete-modal"]')
    .forEach((btn) => {
...
async function addUrl() {
  ...
  try {
    const category =
      document.getElementById("urlCategory").value.trim() || null;
    const description =
      document.getElementById("urlDescription").value.trim() || null;
    ...
    const urlDataList = urls.map((url, index) => {
      ...
      if (!finalTitle) {
        finalTitle =
          urls.length > 1 ? `${timestampBase}_${index + 1}` : timestampBase;
...
async function updateUrl(e) {
  ...
  const urlData = {
    ...
    description:
      document.getElementById("editUrlDescription").value.trim() || null,
  };

  try {
    const { error } = await supabase.from("urls").update(urlData).eq("id", id);
...
function showEditMemoModal(memo) {
...
function renderUrls(urls) {
  elements.urlList.innerHTML = urls
    .map(
      (url) => `
        <div class="url-item">
...
async function findDuplicates() {
  try {
    ...
    const duplicateGroups = Array.from(urlGroups.values()).filter(
...
async function deleteSelectedInGroup(groupIndex) {
  ...
  try {
    ...
    showToast(
      `${idsToDelete.length}개의 중복 항목이 삭제되었습니다.`,
      "success",
    );
      ...
      if (folder._bookmarks && folder._bookmarks.length > 0) {
        folder._bookmarks.forEach((url) => {
          const timestamp = Math.floor(
            new Date(url.created_at).getTime() / 1000,
          );
...
function renderBulkDeleteGrid() {
  elements.bulkDeleteGrid.innerHTML = bulkDeleteItems
    .map(
      (item, index) => `
...
function updateBulkDeleteCount() {
  const selectedItems = document.querySelectorAll(
    '.bulk-delete-item input[type="checkbox"]:checked',
  );
...
async function deleteBulkItems() {
  ...
  const confirmMessage =
    idsToDelete.length === bulkDeleteItems.length
  ...
  try {
    ...
    elements.deleteBulkBtn.innerHTML =
      '<i class="bi bi-hourglass-split"></i> 삭제 중...';

    // Delete in batches
    const batchSize = 50;
    for (let i = 0; i < idsToDelete.length; i += batchSize) {
      const batch = idsToDelete.slice(i, i + batchSize);
      const { error } = await supabase.from("urls").delete().in("id", batch);
    ...
    // Reset button
    elements.deleteBulkBtn.innerHTML =
      '<i class="bi bi-trash"></i> 선택한 항목 삭제';
  } catch (error) {
